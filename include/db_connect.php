...
// 1. Get Author Profile Info
function getAuthorProfile($conn, $author_id) {
    $first_name = '';
    $email = '';
    $stmt = $conn->prepare("
...
// 2. Paper Stats
function getAuthorPaperStats($conn, $author_id) {
    $total = 0;
    $under_review = 0;
    $accepted = 0;
    $rejected_pre = 0;
    ...
    $stmt->bind_param("i", $author_id);
    $stmt->execute();
    $stmt->bind_result($total, $under_review, $accepted, $rejected_pre, $rejected_post);
    $stmt->fetch();
    $stmt->close();

    return compact('total', 'under_review', 'accepted', 'rejected_pre', 'rejected_post');
}

// 3. DOI Published Count
function getAcceptedWithDoiCount($conn, $author_id) {
    $count = 0;
    $stmt = $conn->prepare("
        SELECT COUNT(*) 
        FROM papers 
        WHERE author_id = ? AND doi IS NOT NULL 
        AND (status = 'Accepted(Final Decision)' OR status = 'Published')
    ");
    $stmt->bind_param("i", $author_id);
    $stmt->execute();
...
// 4. Papers Needing Payment
function getPapersNeedingPayment($conn, $author_id) {
    $stmt = $conn->prepare("
        ...
        AND (pay.payment_status IS NULL OR pay.payment_status = 'Pending')
    ");
    $stmt->bind_param("i", $author_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getPapersByChiefEditor($conn, $editorId) {
    $stmt = $conn->prepare("
        ...
        JOIN users u ON a.user_id = u.id
        JOIN journals j ON p.journal_id = j.id
        JOIN editorial_team_members etm ON j.editorial_team_id = etm.team_id
        WHERE etm.editor_id = ? AND etm.role = 'Chief Editor'
          AND p.status NOT IN (
              'Rejected (Pre-Review)', 
        ...
        if (password_verify($password, $user['password'])) {
            ...
            if ($reviewerResult->num_rows === 1) {
                ...
                switch ($reviewer['registration_status']) {
                    ...
                    case 'pending': return "Your application is under review .Once it gets approved, You will recieve an email";
...
// Fetch contract details for an editor
function getEditorContractDetails($conn, $editor_id) {
...
function updateEditorContractFile($conn, $editor_id, $file_name) {
    $upload_date = date("Y-m-d");
    $query = $conn->prepare("UPDATE editors SET contract_status = 'pending_verification', contract_file = ?, upload_date = ? WHERE editor_id = ?");
    $query->bind_param("ssi", $file_name, $upload_date, $editor_id);
...
function insertReviewer($conn, $data) {
    $query = "INSERT INTO reviewers (
        user_id, title, telephone, degree, address, gender, reviewer_type,
        position, institution, department, street_address, city, state,
        zip_code, country, experience, review_frequency, payment_type,
   ...
   $stmt->bind_param(
    ...
    $data['country'],               // s
    $data['experience'],            // i
    $data['review_frequency'],      // i
    $data['payment_type'],          // s
    $data['account_holder_name'],   // s
    $data['bank_name'],             // s
    $data['account_number'],        // s
    $data['ifsc_code'],             // s
    $data['branch_name'],           // s
    $data['bank_country'],          // s
    $data['cv_path']                // s
...
function getManuscripts($conn, $editor_id, $selected_journal, $selected_status, $selected_reviewer, $paper_id = null) {
    $manuscripts_query = "
        SELECT 
            p.id, p.journal_id, p.author_id, p.title, p.abstract, p.file_path, 
            p.submission_date, p.keywords, p.cover_letter_path, 
            p.copyright_agreement_path, p.supplementary_files_path, p.status,
            j.journal_name,
            CONCAT(u_author.first_name, ' ', COALESCE(u_author.middle_name, ''), ' ', u_author.last_name) AS author_name,
            u_review.first_name AS reviewer_first_name,
            u_review.last_name AS reviewer_last_name,
        ...
        LEFT JOIN reviewers r ON pa.reviewer_id = r.id
        LEFT JOIN users u_review ON r.user_id = u_review.id
        INNER JOIN editorial_team_members etm ON j.editorial_team_id = etm.team_id
        WHERE etm.editor_id = " . intval($editor_id);
