...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['check_email'])) {
        ...
        $user_id = getUserIdByEmail_Editor($conn, $email);

        if ($user_id && isEditor($conn, $user_id)) {
            $_SESSION['user_id'] = $user_id;
            $step = 2;
        } else {
            $error = "Email not found or not a Editor.";
            $step = 1;
        }
        ...
        if ($password !== $confirm_password) {
            ...
            if (updateUserPassword_editor($conn, $user_id, $hashed_password)) {
                session_destroy();
