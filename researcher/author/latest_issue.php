...
// Get the latest completed journal
$latestJournal = getLatestJournalIssue($conn, $journal_id);

if ($latestJournal) {
    $latestVolume = $latestJournal['volume'];
    $latestIssue = $latestJournal['issue'];
    $journalId = $latestJournal['journal_id'];

    // Fetch issues in this volume
    $issueResults = getVolumeIssues($conn, $latestVolume, $journalId);
} else {
...
// Get the current page from the URL (defaults to 1 if not set)
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$papersPerPage = 2; // Number of papers per page
$offset = ($page - 1) * $papersPerPage; // Calculate offset for pagination

// Fetch papers in the latest volume and issue with pagination
$paperResults = getPapersByVolumeAndIssue($conn, $latestVolume, $latestIssue, $journalId, $papersPerPage, $offset);

// Fetch total number of papers to calculate total pages
$totalPapers = getTotalPapersCount($conn, $latestVolume, $latestIssue, $journalId);
...
<div style="margin: 20px 0 0 20px;">
    ...
    <div class="container" style="display: flex; gap: 40px; margin: 40px;">
    <!-- LEFT COLUMN -->
    <div style="flex: 1;">
        <h2>Latest Issue</h2>
        <p><strong>Volume <?php echo $latestVolume; ?> — Issue <?php echo $latestIssue; ?></strong></p>
        <button onclick="toggleIssues()" style="margin: 10px 0;">Show All Issues</button>
    ...
    <!-- RIGHT COLUMN -->
<div style="flex: 1;">
    <h2>Papers in Volume <?php echo htmlspecialchars($latestVolume); ?> — Issue <?php echo htmlspecialchars($latestIssue); ?></h2>
<p style="color:red;">Access Type = <?php echo $journal['access_type']; ?></p>

    <?php if ($paperResults->num_rows > 0): ?>
        <div id="papers-list">
           <form id="paperActionForm" method="POST">
    <input type="hidden" name="journal_id" value="<?php echo $journal_id; ?>">
    <input type="hidden" name="access_type" id="accessType" value="<?php echo $journal['access_type']; ?>">
   <input type="hidden" name="volume" value="<?php echo htmlspecialchars($latestVolume); ?>">
<input type="hidden" name="issue" value="<?php echo htmlspecialchars($latestIssue); ?>">
    <div style="margin-top: 15px;">
        <button type="button" onclick="handleDownload()" style="padding: 10px 20px; background-color: #007BFF; color: white; border: none; border-radius: 4px;">
            Download Selected / Proceed to Checkout
        </button>
        <p id="limitWarning" style="color: red; display: none;">You can select a maximum of 5 papers.</p>
    </div>

    <?php while ($row = $paperResults->fetch_assoc()): ?>
        <div class="paper-card">
            <label style="display: flex; align-items: center; gap: 10px;">
                <input type="checkbox" name="selected_papers[]" value="<?php echo $row['id']; ?>">
                <h3 style="margin: 0;">
                    <a href="paper.php?paper_id=<?php echo $row['id']; ?>"><?php echo htmlspecialchars($row['title']); ?></a>
                </h3>
            </label>
            <p><strong>Author:</strong> <?php echo htmlspecialchars($row['author_first_name']) . " " . htmlspecialchars($row['author_last_name']); ?></p>
            <p><strong>Co-Authors:</strong>
                <?php
                    $coAuthors = getCo_Authors1($conn, $row['id'], $row['author_id']);
                    echo implode(', ', $coAuthors);
                ?>
            </p>
        </div>
    <?php endwhile; ?>
</form> <!-- CLOSES after while loop ends -->
        ...
        <div id="pagination">
            <?php if ($page > 1): ?>
                <a href="journal_detail.php?journal_id=<?php echo $journal_id; ?>&page=<?php echo $page - 1; ?>">Previous</a>
            <?php endif; ?>
            <?php if ($page < $totalPages): ?>
                <a href="journal_detail.php?journal_id=<?php echo $journal_id; ?>&page=<?php echo $page + 1; ?>">Next</a>
            <?php endif; ?>
        </div>

    <?php else: ?>
        <p>No papers available in this volume and issue.</p>
    <?php endif; ?>
</div>
</div>
...
<script>
    function toggleIssues() {
        ...
        issueList.style.display = issueList.style.display === "block" ? "none" : "block";
    }
</script>
<script>
function handleDownload() {
    const accessType = document.getElementById('accessType').value.trim().toLowerCase(); // lowercase check
    const checkboxes = document.querySelectorAll('input[name="selected_papers[]"]:checked');
    ...
    if (accessType === 'open access') {
        if (selectedCount > 5) {
            ...
            document.getElementById("paperActionForm").action = "download_papers.php";
            document.getElementById("paperActionForm").submit();
        }
    } else if (accessType === 'subscription based') {
    document.getElementById("paperActionForm").action = "checkout.php";
    document.getElementById("paperActionForm").submit();
} else {
        alert("Unknown access type: " + accessType);
    }
}
</script>
