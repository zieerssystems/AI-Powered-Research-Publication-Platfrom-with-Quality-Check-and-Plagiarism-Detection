...
// Mark manuscripts as visited only when manuscripts.php is opened
$_SESSION['manuscripts_visited'] = true;
...
// Fetch distinct journal names for filter dropdown
$journals_result = getDistinctJournalNames($conn);

// Fetch manuscripts
$manuscripts = getManuscripts($conn, $editor_id, $selected_journal, $selected_status, $selected_reviewer, $paper_id);

// Check for new "Pending" manuscripts
$pending_count = getPendingManuscriptCount($conn, $editor_id);
?>
            ...
            <tbody>
                <?php while ($row = $manuscripts->fetch_assoc()) { ?>
                    <tr>
                        ...
                        <td><?php echo $row['reviewer_first_name'] ? "✅ Assigned to: " . $row['reviewer_first_name'] . " " . $row['reviewer_last_name'] : "❌ Not Assigned"; ?></td>
                        <td>
                        ...
                        <td>
                            <?php if (!empty($row['cover_letter_path'])): ?>
                                <a class="file-link" href="../../uploads/<?php echo basename($row['cover_letter_path']); ?>" target="_blank">View</a>
                            <?php else: ?>
                                N/A
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if (!empty($row['copyright_agreement_path'])): ?>
                                <a class="file-link" href="../../uploads/<?php echo basename($row['copyright_agreement_path']); ?>" target="_blank">View</a>
                            <?php else: ?>
                                N/A
                            <?php endif; ?>
                        </td>
