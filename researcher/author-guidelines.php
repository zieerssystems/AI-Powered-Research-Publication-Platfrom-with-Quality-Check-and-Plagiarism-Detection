...
// Assuming $conn is already initialized in db_connect.php

if (!isset($_GET['journal_id'])) {
    die("Error: journal_id is missing in the URL.");
}

$journal_id = intval($_GET['journal_id']);

// Fetch journal details
$journal = getJournalDetail($conn, $journal_id);
if (!$journal) {
    die("No journal found with the given ID.");
}

// Fetch author guidelines
$author_guidelines = getAuthorGuidelines($conn, $journal_id);
if ($author_guidelines === null) {
    die("No author guidelines found for this journal.");
}
$author_guidelines = htmlspecialchars($author_guidelines);

// Now you can continue your HTML output as before...
...
.editorial-table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        ...
        .editorial-table th { background-color: #1A3A6C; color: white; font-size: 18px; }
        .position { color: #1A3A6C; font-weight: bold; }
        .author-guidelines {
    background-color: #E6F7FF; /* Light Blue Background */
    padding: 20px;
    margin: 20px auto;
    max-width: 900px;
    border: 2px solid #B3E0FF; /* Slightly Darker Border */
    border-radius: 10px;
...
.author-guidelines h3 {
    font-size: 22px;
    color: #005A8D; /* Dark Blue */
    margin-bottom: 15px;
    font-weight: bold;
    text-align: center;
}
...
<div style="margin: 20px 0 0 20px;">
    ...
    <div class="container">
    <h2 style="color: #1A3A6C; text-align: center; font-weight: bold;">Author Guidelines</h2>
    <div class="author-guidelines">
        <?php echo nl2br($author_guidelines); ?>
    </div>
</div>
</div>
    </div>
