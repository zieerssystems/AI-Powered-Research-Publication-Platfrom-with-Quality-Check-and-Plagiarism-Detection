...
<script>
        ...
        function attachContract(editorId) {
            const input = document.getElementById('contract_' + editorId);
            if (!input.files.length) {
                alert('Please choose a contract file first.');
                return;
            }
            const form = document.getElementById('form_' + editorId);
            form.submit();
        }
    </script>
</head>
...
<td><?= htmlspecialchars($editor['email'] ?? ''); ?></td>
                        ...
                        <td>
                            <?php if (!empty($editor['cv_path'])): ?>
                                <a href="/../<?= htmlspecialchars($editor['cv_path']); ?>" target="_blank" class="btn email-btn">View</a>
<?php else: ?>
    ...
    <form action="send_email.php" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 4px; align-items: center;">
        <input type="hidden" name="editor_id" value="<?= $editor['editor_id']; ?>">
        <input type="file" name="contract" accept="application/pdf" required onchange="toggleSendBtn(this)">
        <button type="submit" class="btn email-btn" disabled>Send</button>
    </form>
