...
$journal = fetchJournalDetails($conn, $journal_id);
...
// Step 4: Fetch scope details
$scopeDetails = fetchScopeDetails($conn, $journal_id);

if (!$scopeDetails) {
    die("No scope details found for this journal.");
}

$scope = htmlspecialchars($scopeDetails['scope']);
?>
...
.user-dropdown:hover .dropdown-menu {
    display: block;
        ...
        .scope-table th {
            background-color: rgb(121, 157, 216);
            color: white;
            font-size: 18px;
            width: 20%;
        }
