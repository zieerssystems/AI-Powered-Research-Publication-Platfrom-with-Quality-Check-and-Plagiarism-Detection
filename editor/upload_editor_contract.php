...
if (!isset($_GET['editor_id']) || empty($_GET['editor_id'])) {
   ...
   $row = getEditorContractDetails($conn, $editor_id);
if ($row) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $editor_id && isset($_FILES['signed_contract'])) {
    ...
    $upload_dir = __DIR__ . "/../admin/contracts/signed/";
    
    if (!is_dir($upload_dir)) {
        mkdir($upload_dir, 0777, true);
    }

    // rest of your upload code continues here...
    ...
    if ($file_ext !== 'pdf') {
        ...
        if (move_uploaded_file($file_tmp, $target_file)) {
            ...
            if (updateEditorContractFile($conn, $editor_id, $new_file_name)) {
