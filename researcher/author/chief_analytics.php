...
// ✅ Use functions to fetch data
$total_manuscripts = getTotalManuscripts($conn);
$pending_reviews = getPendingReviews($conn);
$accepted_papers = getAcceptedPapers($conn);
$rejected_papers = getRejectedPapers($conn);

$total_reviewers = getTotalReviewers($conn);
$active_reviewers = getActiveReviewers($conn);
$top_authors = getTopAuthors($conn);

$journal_data = getJournalStats($conn);
$journals = [];
$accepted_counts = [];
$rejected_counts = [];
